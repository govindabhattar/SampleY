...
public class WebViewActivityRazorPay extends BaseActivity implements Keys.Extras, View.OnClickListener {
...
//        super.onBackPressed();
    ...
    private boolean onOverrideUrlLoading(WebView view, String url) {
        ...
        String rzpPaymentId = sanitizer.getValue("rzp_payment_id");
...
//        newView.setLayoutParams(new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        view.loadUrl(url);
//
//        Log.e("wvWebsite.getChildCount()", webView.getChildCount() + "");
//        webView.addView(newView);
//        Log.e("wvWebsite.getChildCount()", webView.getChildCount() + "");
        if (url.toLowerCase().contains("success") && (rzpPaymentId != null && !rzpPaymentId.isEmpty())) {

            Bundle extras = new Bundle();
            extras.putBoolean("hitFetchMerchantCard", true);
            if (rzpPaymentId != null && !rzpPaymentId.isEmpty()) {
                extras.putString("rzpPaymentId", rzpPaymentId);
            }
            Intent intent = new Intent();
            intent.putExtras(extras);
            setResult(RESULT_OK, intent);
