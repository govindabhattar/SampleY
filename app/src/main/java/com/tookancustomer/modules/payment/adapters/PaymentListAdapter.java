package com.tookancustomer.modules.payment.adapters;
...
import static com.tookancustomer.appdata.Keys.Extras.CARD_DATA;
...
public class PaymentListAdapter extends RecyclerView.Adapter<PaymentListAdapter.ViewHolder> {
    ...
    private boolean showPaymentMethodSelection;
    private boolean showLowBalanceText;
    private double payableAmount;
    private PaytmData paytmData;
    private Callback mCallback;

    public PaymentListAdapter(Activity activity, List<Datum> dataList, boolean showPaymentMethodSelection, boolean showLowBalanceText
            , double payableAmount, PaytmData paytmData, Callback mCallback) {
        this.activity = activity;
        this.dataList = dataList;
        this.showPaymentMethodSelection = showPaymentMethodSelection;
        this.showLowBalanceText = showLowBalanceText;
    ...
    public void onBindViewHolder(final ViewHolder holder, int position) {
        ...
        if (showPaymentMethodSelection) {
            ...
            /* Setting payment method text for payment methods except paytm
             * and logo for paytm */
        ...
        if (dataList.get(adapterPos).getPaymentMethod() == PAYTM.intValue && paytmData.getPaytmVerified() != null && paytmData.getPaytmVerified() == 1) {
            holder.pbLoading.setVisibility(View.GONE);
            holder.tvAmount.setVisibility(View.VISIBLE);
            holder.tvAmount.setText(Utils.getCurrencySymbol() + Utils.getDoubleTwoDigits(paytmData.getWalletBalance()));

            if (showLowBalanceText && payableAmount > paytmData.getWalletBalance()) {
                holder.tvSubHeadingText.setVisibility(View.VISIBLE);
                holder.tvSubHeadingText.setText(StorefrontCommonData.getString(activity, R.string.low_balance_add_funds).replace("$$$", Utils.getCurrencySymbol() + Utils.getDoubleTwoDigits(payableAmount - paytmData.getWalletBalance())));
            } else {
                holder.tvSubHeadingText.setVisibility(View.GONE);
            }

        } else if (dataList.get(adapterPos).getPaymentMethod() == PAYTM.intValue && paytmData.getPaytmVerified() != null && paytmData.getPaytmVerified() == 0) {
            holder.pbLoading.setVisibility(View.GONE);
            holder.tvAmount.setVisibility(View.VISIBLE);
        ...
        holder.rlAddCard.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                mCallback.onPaymentMethodSelected(adapterPos, dataList.get(adapterPos));
            }
        });
        ...
        holder.ivMore.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showMoreOptions(dataList.get(adapterPos), holder.ivMore);
            }
        });
        ...
        holder.tvAmount.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (dataList.get(adapterPos).getPaymentMethod() == PAYTM.intValue && paytmData.getPaytmVerified() != null && paytmData.getPaytmVerified() == 0) {
                    PaymentManager.requestOtpForPaytm(activity);
                } else if (dataList.get(adapterPos).getPaymentMethod() == PAYTM.intValue && paytmData.getPaytmVerified() != null && paytmData.getPaytmVerified() == 1 && payableAmount > paytmData.getWalletBalance()) {
                    PaymentManager.openAddPaytmMoneyWebview(activity, paytmData.getPaytmVerified(), paytmData.getPaytmAddMoneyUrl());
                } else {
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private ImageButton ivSelectedCard;

        private ImageView ivMore;

        private TextView tvAmount, tvSubHeadingText;
    ...
    public interface Callback {
        void onPaymentMethodSelected(int adapterPos, Datum datum);
    }

    public void showMoreOptions(final Datum datum, View view) {
        PopupMenu popup;
        ...
        popup.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
            public boolean onMenuItemClick(MenuItem item) {
                if (item.getTitle().equals(StorefrontCommonData.getString(activity, R.string.view_menu_option))) {
                    ...
                    intent.putExtra(CARD_DATA, datum);
    ...
    private void deleteCard(final Datum datum) {
        CommonParams.Builder commonParams = Dependencies.setCommonParamsForAPI(activity, StorefrontCommonData.getUserData());
        commonParams.add("card_id", datum.getCardId());

        if (datum.getPaymentMethod() == AUTHORISE_DOT_NET.intValue) {
            commonParams.add("is_active", "0")
                    .add("last4_digits", datum.getLast4Digits())
                    .add("expiry_date", datum.getExpiryDate())
                    .add("brand", datum.getBrand())
                    .build();

            RestClient.getApiInterface(activity).deleteMerchantCards_AuthorizeDotNet(commonParams.build().getMap()).enqueue(getDeleteCardResponse(datum));
        } else if (datum.getPaymentMethod() == FAC.intValue) {
            ...
            commonParams.add("is_active", "0")
                    .add("card_id", datum.getCardId())
                    .add("payment_method", datum.getPaymentMethod())
                    .add("last4_digits", datum.getLast4Digits())
                    .add("expiry_date", datum.getExpiryDate())
                    .add("brand", datum.getBrand())
                    .build();

            RestClient.getApiInterface(activity).deleteMerchantCards_FAC(commonParams.build().getMap()).enqueue(getDeleteCardResponse(datum));

        } else if (datum.getPaymentMethod() == VISTA_MONEY.intValue) {
            ...
            commonParams.add("payment_method", datum.getPaymentMethod())
                    .build();

            RestClient.getApiInterface(activity).deleteMerchantCards(commonParams.build().getMap()).enqueue(getDeleteCardResponse(datum));
        }
    }

    private retrofit2.Callback<BaseModel> getDeleteCardResponse(final Datum datum) {
