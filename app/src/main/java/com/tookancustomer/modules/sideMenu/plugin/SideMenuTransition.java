...
//                FuguConfig.getInstance().showConversations(mActivity, StorefrontCommonData.getString(mActivity, R.string.support));
                break;
    ...
    public static void setSliderUI(final Activity mActivity, UserData userData) {
        check = 0;
        optionsAdapter = new OptionsAdapter(getSideMenuOptions(userData, mActivity));
        RecyclerView recyclerView = ((RecyclerView) mActivity.findViewById(R.id.recyclerOptions));
...
/*                        Utils.snackBar(CheckOutActivity.this, getStrings(R.string.please_enter_pickup_address_v2).replace(TerminologyStrings.ADDRESS, StorefrontCommonData.getTerminology().getAddress()));
            ...
            ((TextView) mActivity.findViewById(R.id.tvMyAddresses)).setText(StorefrontCommonData.getString(mActivity, R.string.my_addresses));
...
/*
            mActivity.findViewById(R.id.llPaymentMethodLayout).setVisibility(PaymentMethodsClass.isPaymentEnabled() ? View.VISIBLE : View.GONE);
...
//                }
    ...
    private static ArrayList<SideMenu> getSideMenuOptions(UserData userData, Activity mActivity) {
        ArrayList<SideMenu> sideMenuList = new ArrayList<>();

        sideMenuList.add(new SideMenu(Utils.getCallTaskAs(userData, false, true), R.drawable.ic_orders));

        if (PaymentMethodsClass.isInAppWalletPaymentEnabled())
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getWallet(), R.drawable.ic_wallet));

        if (StorefrontCommonData.getAppConfigurationData().getIsGiftCardActivated() == 1 &&
                PaymentMethodsClass.isInAppWalletPaymentEnabled())
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getGiftCard(), R.drawable.ic_gift_card));

        if (PaymentMethodsClass.isPaymentEnabled())
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getPayment(true), R.drawable.ic_payments));

        if (userData.getData().getReferral().getStatus() == 1 && Dependencies.getToggleView(mActivity) == 0)
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getReferral(true), R.drawable.ic_referrals));

        if (UIManager.getIsLoyaltyEnable() == 1)
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getLoyaltyPoints(), R.drawable.ic_money_bag));

        sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getNotifications(true), R.drawable.ic_notification_bell));
        sideMenuList.add(new SideMenu(StorefrontCommonData.getString(mActivity, R.string.my_addresses).replace(ADDRESS,StorefrontCommonData.getTerminology().getAddress()), R.drawable.ic_icon_pin_location));

        if (UIManager.isFuguChatEnabled() && Dependencies.getToggleView(mActivity) == 0)
            sideMenuList.add(new SideMenu(StorefrontCommonData.getString(mActivity, R.string.chat_with_fugu), R.drawable.ic_chat));


        if (StorefrontCommonData.getAppConfigurationData().getIsDynamicPagesActive() == 1 &&
                StorefrontCommonData.getAppConfigurationData().getDynamicPagesDetails().size() > 0)
            sideMenuList.add(new SideMenu(StorefrontCommonData.getTerminology().getDynamicPages(), R.drawable.ic_document));


        sideMenuList.add(new SideMenu("Rewards", R.drawable.ic_document));
