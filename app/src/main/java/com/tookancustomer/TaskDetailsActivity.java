...
public class TaskDetailsActivity extends BaseActivity implements View.OnClickListener {
    ...
    private RelativeLayout rlOrderStatus, rlSummaryHeader, rlOrderTime, rlOrderEndTime, rlPaymentStatus, rlPaymentMode, rlRefundAmount, rlRemainingAmount;
    ...
    private RelativeLayout rlTotalAmount, rlTotalPayableAmount;
    ...
    private CardView cvRateReviews, cvPointsEarned, cvRemainingAmount, cvAdditionalPriceParent;
    ...
    private void initViews() {
        ...
        Utils.setOnClickListener(this, rlBack, tvRetry, btnCancelOrder, tvAdditionalHeading, tvAdditionalHeadingCustomNoPrice,
                tvCancellationPolicy, btnSubmitRate, tvRepay, btnChatWithUs, btnTrack);
    }
    ...
    public void setData() {
        ...
        if (!(currentTask.getPaymentType() == UNACCOUNTED.intValue)) {
            ...
            tvPaymentMode.setText(PaymentConstants.PaymentValue.getPaymentString(mActivity, currentTask.getPaymentType(), ""));
        } else {
...
////            }
        ...
        if (currentTask.getTaskType() == Constants.LaundryTaskType.DELIVERY_TASK) {
            ...
            tvPickupAddressLabel.setText(getStrings(R.string.pickup_address_v2).replace(PICKUP, StorefrontCommonData.getTerminology().getPickup(true)
                    .replace(TerminologyStrings.ADDRESS, StorefrontCommonData.getTerminology().getAddress())) + getStrings(R.string.colon));
        } else {
        ...
        if (currentTask.getIsCustomOrder() == 1 || currentTask.getIsCustomOrder() == 2) {
            ...
            final ViewGroup.LayoutParams lparams = etDescription.getLayoutParams();
            lparams.height = ViewGroup.LayoutParams.WRAP_CONTENT;
            etDescription.setLayoutParams(lparams);
...
//                    tvEndDateTimeLabel.setText(getStrings(R.string.scheduled_time) + " :");
                    ...
                    if (currentTask.getTaskType() == Constants.TaskTypeConstants.FOOD) {
                        ...
                        pickupDate =
                                DateUtils.getInstance().parseDateAs(currentTask.getJobPickupDatetime(), BACKEND_PICKUP_TIME, UIManager.getDateTimeFormat());
       ...
       // ArrayList<TaxesModel> billBreakdownlist = new ArrayList<>();
        ...
        if (currentTask.getIsCustomOrder() == 2) {
            billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.subtotal), BigDecimal.valueOf(Double.valueOf(currentTask.getOrderAmount())),currentTask.getDeliveryChargeSurgeAmount()));
        } else {
            billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.subtotal), BigDecimal.valueOf(Double.valueOf(getOrderAmount(currentTask, false))),currentTask.getDeliveryChargeSurgeAmount()));
        }

        if (currentTask.getTransaction_charges() > 0) {

            billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.transaction_charges), BigDecimal.valueOf(Double.valueOf(currentTask.getTransaction_charges())),currentTask.getDeliveryChargeSurgeAmount()));

        }

        if (isDeliveryPromo) {
            billBreakdownDataArrayList.add(new BillBreakdownData(StorefrontCommonData.getTerminology().getDeliveryCharge(),
                    BigDecimal.valueOf(Double.valueOf(currentTask.getDeliveryCharge())), deliveryPromoDescription,currentTask.getDeliveryChargeSurgeAmount()));
        } else if (Double.valueOf(currentTask.getDeliveryCharge()) > 0) {
            billBreakdownDataArrayList.add(new BillBreakdownData(StorefrontCommonData.getTerminology().getDeliveryCharge(),
                    BigDecimal.valueOf(Double.valueOf(currentTask.getDeliveryCharge())),currentTask.getDeliveryChargeSurgeAmount()));
        }

      //  billBreakdownDataArrayList.addAll(currentTask.getUserTaxes());
        if (currentTask.getUserTaxes().size() > 0) {

            for (int i = 0; i < currentTask.getUserTaxes().size(); i++) {
                billBreakdownDataArrayList.add(new BillBreakdownData(currentTask.getUserTaxes().get(i).getTaxName(),
        ...
        if (Double.valueOf(currentTask.getTip()) > 0) {
            billBreakdownDataArrayList.add(new BillBreakdownData(StorefrontCommonData.getTerminology().getTip(), BigDecimal.valueOf(Double.valueOf(currentTask.getTip())),currentTask.getDeliveryChargeSurgeAmount()));
        }

        if (Double.valueOf(currentTask.getCouponDiscount()) > 0) {
            if (isSubtotalPromo) {
                billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.discount), BigDecimal.valueOf(Double.valueOf(currentTask.getCouponDiscount())), true, subtotalPromoDescription,currentTask.getDeliveryChargeSurgeAmount()));
            } else {
                billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.discount), BigDecimal.valueOf(Double.valueOf(currentTask.getCouponDiscount())), true,currentTask.getDeliveryChargeSurgeAmount()));
            }
        }
        ...
        if (currentTask.getLoyaltyPoints() != null && currentTask.getLoyaltyPoints().getDiscountAmount() != null) {
            if (currentTask.getLoyaltyPoints().getPointsRedeemed() > 0) {
                billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.loyalty_points_used_amount).replace(LOYALTY_POINTS, StorefrontCommonData.getTerminology().getLoyaltyPoints()).replace(AMOUNT, currentTask.getLoyaltyPoints().getPointsRedeemed() + "")
                        , BigDecimal.valueOf(currentTask.getLoyaltyPoints().getDiscountAmount().doubleValue()), true,currentTask.getDeliveryChargeSurgeAmount()));
            }
        }


        if (currentTask.getRefundedAmount() > 0) {
            billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.refunded_amount),
                    new BigDecimal(currentTask.getRefundedAmount()), true,currentTask.getDeliveryChargeSurgeAmount()));
        }
        ...
        if (Dependencies.isLaundryApp() && currentTask.getTaskType() == 2) {
            if (!Utils.getDoubleTwoDigits(currentTask.getRemainingBalance().doubleValue()).equalsIgnoreCase("0.00")) {
                if (currentTask.getRemainingBalance().doubleValue() > 0) {
                    billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.remaining_amount),
                            currentTask.getRemainingBalance(),currentTask.getDeliveryChargeSurgeAmount()));
                } else {
                    billBreakdownDataArrayList.add(new BillBreakdownData(getStrings(R.string.remaining_amount),
                            new BigDecimal(-1 * currentTask.getRemainingBalance().doubleValue()), true,currentTask.getDeliveryChargeSurgeAmount()));
                }
            }
        ...
        if (StorefrontCommonData.getFormSettings().getShowProductPrice() == 0 && Double.valueOf(getOrderAmount(currentTask, true)) <= 0) {
            ...
            if (currentTask.getLoyaltyPoints() != null) {
                ...
                if (currentTask.getLoyaltyPoints().getDiscountAmount() != null) {
                    rlTotalPayableAmount.setVisibility(View.GONE);
                    tvTotalPayable.setText((UIManager.getCurrency(Utils.getCurrencySymbol() + getPayableAmount(orderAmount, currentTask))));
                }
        ...
        if (Dependencies.isLaundryApp() && currentTask.getTaskType() == 2) {
            if (!Utils.getDoubleTwoDigits(currentTask.getRemainingBalance().doubleValue()).equalsIgnoreCase("0.00")) {
                ...
                tvRemainingAmount.setText((UIManager.getCurrency(Utils.getCurrencySymbol() + Utils.getDoubleTwoDigits(currentTask.getRemainingBalance().doubleValue()))));
            } else {
        ...
        if (currentTask.getRefundedAmount() > 0) {
            ...
            tvRefundAmount.setText((UIManager.getCurrency(Utils.getCurrencySymbol() + Utils.getDoubleTwoDigits(currentTask.getRefundedAmount()))));
        } else {
        ...
        tvTotal.setText((UIManager.getCurrency(Utils.getCurrencySymbol() + orderAmount)));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btnChatWithUs:
                if (UIManager.isFuguChatEnabled() && currentTask != null && HippoConfig.getInstance() != null) {
...
//                                currentTask.getGroupingTags());
                        attributes = new ChatByUniqueIdAttributes.Builder()
                                .setTransactionId(currentTask.getHippoTransactionId())
                                .setUserUniqueKey(StorefrontCommonData.getUserData().getData().getVendorDetails().getVendorId().toString())
                                .setChannelName(currentTask.getMerchantName() + " #" + currentTask.getJobId())
                        ...
                        attributes = new ChatByUniqueIdAttributes.Builder()
                                .setTransactionId(currentTask.getJobId().toString())
                                .setUserUniqueKey(StorefrontCommonData.getUserData().getData().getVendorDetails().getVendorId().toString())
