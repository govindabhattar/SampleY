...
public class CustomFieldImageProfile implements SignupTemplateData.Listener, View.OnClickListener {
    ...
    public CustomFieldImageProfile(Context context) {
        ...
        rvAddedImages = (RecyclerView) view.findViewById(R.id.rvImages);
        ...
        Utils.setOnClickListener(this, vCustomFieldIcon);
    }
     ...
     * @return
     */
    public View render(SignupTemplateData item,boolean isEnable) {
        this.item = item;
        ...
        if (!item.getData().toString().isEmpty() && item.getData() instanceof ArrayList) {
            ...
            if (imagesList.size() > 0) {
                ...
                rvAddedImages.setVisibility(View.VISIBLE);
                rvAddedImages.setAdapter(customFieldImageAdapter = new CustomFieldImagesAdapterProfile(activity, imagesList, item,isEnable));
            } else {
        ...
        if (item.isReadOnly()) {
            vCustomFieldIcon.setEnabled(false);
        }
    ...
    public void onClick(View view) {
        ...
        switch (view.getId()) {
            case R.id.vCustomFieldIcon:
                ...
                if (item.getAttribute()!=null&&item.getAttribute() == Constants.ImageType.CAMERA_ONLY) {
                    imageUtils.startCamera(Codes.Request.OPEN_CAMERA_CUSTOM_FIELD_IMAGE);
                } else if (item.getAttribute()!=null&&item.getAttribute() == Constants.ImageType.GALLERY_ONLY) {
                    imageUtils.openGallery(Codes.Request.OPEN_GALLERY_CUSTOM_FIELD_IMAGE);
                } else {
