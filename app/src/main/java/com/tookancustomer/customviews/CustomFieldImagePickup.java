...
import com.tookancustomer.adapter.CustomFieldImagesAdapterPickup;
...
public class CustomFieldImagePickup implements Item.Listener, View.OnClickListener {

    private final String TAG = CustomFieldImagePickup.class.getSimpleName();
 ...
 //   private TextView tvPlaceHolder;
    ...
    private Item item;
    private CustomFieldImagesAdapterPickup customFieldImageAdapter;


    /**
     * Method to initialize the CustomField
     */
    public CustomFieldImagePickup(Context context) {
    ...
    public View render(Item item) {
        ...
        if (!item.getData().toString().isEmpty() && item.getData() instanceof ArrayList) {
         ...
         //   vSeperator.setVisibility(View.GONE);
            rvAddedImages.setAdapter(customFieldImageAdapter = new CustomFieldImagesAdapterPickup(activity, imagesList, item));
        }
